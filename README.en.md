...
**Plan > Vibes. Three commands: /plan draws the map, /go drives to the end, /fix when it breaks.**

> /plan draws the map. /go drives to the end — it only stops for a missing key or a dangerous move. /fix when it breaks.
...
flowchart TD
    ...
    subgraph L["loop per step — never yields the turn"]
        B["branch go/NN"] --> I[implement the tasks]
...
`/fix` steps in when an Acceptance breaks: `/go` calls it inline. When a bug shows up outside a run, you call it directly.
...
| **/go** | Roadmap ready; you want the steps DONE | Steps N..M executed end to end: 1 step = 1 branch = 1 reviewed PR = 1 merge. With no argument, it's the status panel |
...
What Forger forbade, Katana declares the loop's happy ending: pushing the `go/*` branch, opening a PR, commenting, merging into main **via PR**, deleting the merged branch.

### /go resume

Session died, context overflowed, you closed the laptop? `/go resume` re-reads `.katana/state.json` and reconciles against `gh pr list` + `git log` — **GitHub/git win over the JSON**. It resumes exactly at the pending step. `/go stop` is the kill switch. `/go 3..5 --dry` shows the flight plan without executing.
...
[go 1/3] step 01 merged — PR #1 (1 attempt, 8 min)
[go 2/3] step 02 merged — PR #2 (2 attempts: 1 auto-fix of a test, 11 min)
[go 3/3] step 03 merged — PR #3 (self-review caught an off-by-one, fixed, 15 min)
...
Katana goes back to **3 + 3**: three commands (/plan, /go, /fix), three surfaces in your project (`ROADMAP.md`, `.katana/`, `.env.example`).
...
| **hard stop** (parada dura) | An item from the closed list above; the only way /go stops |
...
- Philosophy: **copying is harmless, activating is explicit opt-in (`/go setup`).** Never ask for `bypassPermissions` — the allowlist + guard give the same flow with a leash on.
...
`/go resume`. State lives in `.katana/state.json` and in the PRs — the session is disposable. The SessionStart hook still injects 1 line of context into the new session.
...
`scripts/go.ps1` (headless): one `claude -p "/go step K" --resume <sid>` invocation PER step, branching on the JSON `subtype` (never on exit code), fail-closed — no status written means the runner stops instead of running blind.

**Why is there no /status?**
`/go` with no argument IS the panel — derived from real files (ROADMAP checkboxes, state.json, git), never from guesswork, and it ends by pointing at the next step. One more command would be one more surface, and the rule is to remove surfaces.

---

**Start now:** `/plan` with your idea, your way → `ROADMAP.md` → `/go 1..N`. When it breaks: `/fix`.
