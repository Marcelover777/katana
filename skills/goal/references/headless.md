# headless — /goal overnight com scripts/goal.ps1

O modo primário do /goal é a sessão interativa. Headless existe pra madrugada: `scripts/goal.ps1`
...
retome de manhã com `/goal resume`.

## As 5 leis do runner

1. **Uma invocação por etapa.**
   `claude -p "/goal step K" --output-format json --permission-mode acceptEdits`.
...
4. **Sem `--bare`** (mata a descoberta de skill e o CLAUDE.md). **Sem `bypassPermissions`** —
   a allowlist do `/goal setup` + o `katana-guard` cobrem o fluxo inteiro.
5. **Pare quando `status` ∈ {done, hard_stop, stopped}.** E um run por vez: lock em
   `.katana/tmp/goal.lock` (já gitignorado; o goal.ps1 recusa iniciar se existir), e `state.json`
   com `status=running` e `updated_at` recente → recuse iniciar.

## /goal step K — a forma que o runner usa

É `/goal K` em modo não-interativo. Diferenças:
...
- Bata TODOS os gates ANTES de dormir, não às 3h: rode `/goal N..M --dry` interativo antes — ele
...
## Esqueleto (o script real vive em scripts/goal.ps1, ~100 linhas)
...
UTF-8 **sem BOM**. Escrito pelo /goal a cada transição; lido pelos hooks (`katana-continue`,
`katana-session-start`) e pelo goal.ps1. GitHub/git são a fonte de verdade remota; o state é o
cache local que o `/goal resume` reconcilia.
...
```json
{
  ...
  "nudges": 0,
  "steps": {
    "1": { "status": "merged",  "branch": "goal/01-watermark-join",   "pr": 192, "sha": "a1b2c3",
           "attempts": 1, "last_error": null, "decisions": [] },
    "2": { "status": "merged",  "branch": "goal/02-origin-budget",    "pr": 193, "sha": "d4e5f6",
           "attempts": 2, "last_error": null, "decisions": [] },
    "3": { "status": "running", "branch": "goal/03-tabular-champion", "pr": null, "sha": null,
...
| `range` | `[N, M]` da faixa pedida (`/goal step K` estende o M) |
...
| `nudges` | contador do Stop hook `katana-continue`; o /goal zera a cada progresso real; ≥3 sem progresso → `hard_stop` com `stalled` |
...
| `steps{K}.branch` / `pr` / `sha` | branch `goal/KK-<slug>`, nº do PR (ou null), sha do merge |
